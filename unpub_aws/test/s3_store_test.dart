...
void main() async {
  group('test s3 file store', () {
    ...
    test('aws-credentials-manual', () async {
      var creds = AwsCredentials(awsAccessKeyId: 'specialKey', awsSecretAccessKey: 'specialSecret');
    ...
    test('aws-credentials-ecs-container-iam', () async {
      ...
      var creds = await AwsCredentials(environment: credMap, containerCredentials: containerCredentials);
    ...
    test('upload-download-default-path', () async {
      ...
      var pkg1 = await readByteStream(testS3Store.download('test_package', '1.0.0'));
      expect(pkg1, testPackageData);
    });

    test('upload-download-custom-path', () async {
      expect(testS3Store.getObjectPath!.call('test_package', '1.0.0'), newFilePathFunc().call('test_package', '1.0.0'));
      expect(testS3Store.getObjectPath!.call('test_package2', '2.0.0'), newFilePathFunc().call('test_package2', '2.0.0'));

      await testS3Store.upload('test_package', '1.0.0', testPackageData2);
      var pkg2 = await readByteStream(testS3Store.download('test_package', '1.0.0'));
